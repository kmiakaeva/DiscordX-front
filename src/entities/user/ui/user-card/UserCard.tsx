...
export function UserCard() {
  const { name, username, status, customStatus, avatarImage } = useUserStore();
  const [showUsername, setShowUsername] = useState(false);

  return (
    <div className="py-1 pr-6 flex space-x-3 rounded cursor-pointer duration-75 ease-in-out hover:bg-zinc-700">
      <AvatarWithBadge className="w-10 h-10" fallback={name[0]} avatarImage={avatarImage} status={status} />
